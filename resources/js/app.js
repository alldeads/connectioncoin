...
jQuery(document).ready(function ($) {
    ...
    $(document).on('click', '.emotions li a', function (e) {
        ...
        var btn = $(this);
        var image = btn.parents().get(1);

        $('.emotions li').removeClass('swing animated');
        $('.emotions li').removeClass('heartBeat animated');
        $('.emotions li').removeClass('bounce animated');
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if ( reaction_id == 1 ) {
                    btn.parent().addClass('swing animated');
                }

                if ( reaction_id == 2 ) {
                    btn.parent().addClass('heartBeat animated');
                }

                if ( reaction_id == 3 ) {
                    btn.parent().addClass('bounce animated');
                }

                if (reaction_id == 4) {
                    $(image).find('img').addClass('tada animated');
                    $(image).find('img').attr('src', '../images/connection_on.jpg');
                }

                setTimeout(function(){ btn.popover('hide'); }, 1000);
            }
        });
